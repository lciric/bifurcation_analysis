...
function [x1,f1,v1,s1,h1,x2,f2,v2,s2,h2]=Equilibrium_Point(path,folder,nb_variable,xinit,MaxStepsize,MaxNumPoints_forward,MaxNumPoints_backward,value,TestTolerance)
    global cds
    global name_variable
    [UserInfo,pvec,syshandle] = start_analyse_Lazar(path,folder,nb_variable);
    ...
    if MaxNumPoints_forward > 0
            ...
            save(path+"/"+folder+"/"+name_variable(nb_variable)+'_Lazar_f.mat','x','v','s','h','f');
    end
    %%
    ...
    %%%%%% to bootstrap the continuation.
     if MaxNumPoints_backward > 0
        ...
        save(path+"/"+folder+"/"+name_variable(nb_variable)+'_Lazar_b.mat','x','v','s','h','f');
     else
         x2 = x1(:,1);
         v2 = v1(:,1);
         f2 = f1(:,1);
    ...
    %% save figures
    savefig(f,path+"/"+folder+"/"+name_variable(nb_variable)+'_Lazar_precision.fig');
    close(f)
    %f=printing(x1,f1,v1,s1,x,f,v,s,nb_variable);
    %% save figures
    %savefig(f,path+"/"+folder+'/p_exc_AB_Lazar_precision.fig');
